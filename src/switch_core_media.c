...
SWITCH_DECLARE(uint8_t) switch_core_media_negotiate_sdp(switch_core_session_t *session, const char *r_sdp, uint8_t *proceed, switch_sdp_type_t sdp_type)
{
	...
	for (m = sdp->sdp_media; m; m = m->m_next) {
		...
		if (got_msrp && m->m_type == sdp_media_message) {
			...
			switch_core_media_set_rmode(smh->session, SWITCH_MEDIA_TYPE_TEXT, SWITCH_MEDIA_FLOW_SENDRECV, sdp_type);
			...
			if (zstr(msrp_accept_types)) {
				...
				switch_channel_set_variable(session->channel, "sip_msrp_local_accept_types", smh->msrp_session->remote_accept_types);
			} else {
				smh->msrp_session->local_accept_types = switch_core_session_strdup(session, msrp_accept_types);
			}
			if (zstr(msrp_accept_wrapped_types)) {
				smh->msrp_session->local_accept_wrapped_types = smh->msrp_session->remote_accept_wrapped_types;
				switch_channel_set_variable(session->channel, "sip_msrp_local_accept_wrapped_types", smh->msrp_session->remote_accept_wrapped_types);
			} else {
				smh->msrp_session->local_accept_wrapped_types = switch_core_session_strdup(session, msrp_accept_wrapped_types);
			}
 ...
 video:
	...
	if (smh->msrp_session) {
		...
		if (!zstr(msrp_session->remote_path)) {
			...
			if (!zstr(msrp_session->local_accept_wrapped_types)) {
				switch_snprintf(buf + strlen(buf), SDPBUFLEN - strlen(buf),
			...
			switch_snprintf(buf + strlen(buf), SDPBUFLEN - strlen(buf),
				...
				msrp_session->secure ? "TLS/" : "",
				msrp_session->local_path,
				zstr(msrp_accept_types) ? "message/cpim text/* application/im-iscomposing+xml" : msrp_accept_types,
				msrp_session->active ? "active" : "passive");

			if (!zstr(msrp_accept_wrapped_types)) {
				switch_snprintf(buf + strlen(buf), SDPBUFLEN - strlen(buf),
					"a=accept-wrapped-types:%s\r\n", msrp_accept_wrapped_types);
			}
																	  ...
																	  int stream_id)
{
	...
	if (switch_core_session_media_flow(session, SWITCH_MEDIA_TYPE_TEXT) == SWITCH_MEDIA_FLOW_RECVONLY || switch_core_session_media_flow(session, SWITCH_MEDIA_TYPE_TEXT) == SWITCH_MEDIA_FLOW_INACTIVE) {
		//switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "Inactive %d\n", switch_core_session_media_flow(session, SWITCH_MEDIA_TYPE_TEXT));
...
SWITCH_DECLARE(switch_status_t) switch_core_session_printf(switch_core_session_t *session, const char *fmt, ...)
{
	...
	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "Text writef\n");
	if (ret == -1) {
		abort();
	}
