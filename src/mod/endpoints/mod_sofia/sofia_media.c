	...
	if ((dval = strchr(dname, ':'))) {
		*dval++ = '\0';
		if (*dval == '~') {
			...
			// If there are headers that will get attached to the multipart body, then we need to recalc the length based on where the double newlines are
			if (bstart != NULL) {
	...
	if ((hi = switch_channel_variable_first(channel))) {
		for (; hi; hi = hi->next) {
			...
			if (!strcasecmp(name, prefix)) {
				// put the sdp first so systems can handle it easier
				if (x == 0) {
